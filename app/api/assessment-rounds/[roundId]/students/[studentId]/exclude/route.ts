...
export async function POST(request: NextRequest, context: Context) {
  ...
  if (!Number.isInteger(expectedRevision) || !key || match !== String(expectedRevision) || typeof body.reason !== "string" || !body.reason.trim() || body.reason.trim().length > 500) return fail("INVALID_REQUEST", 400);
  try {
    ...
    if (!store.getAssessmentRound || !store.excludeStudentRound) return fail("STORAGE_UNAVAILABLE", 503);
    const round = await store.getAssessmentRound(roundId); if (round.schoolId !== teacher.schoolId) return fail("FORBIDDEN", 403);
    const outcome = await store.excludeStudentRound({ roundId, studentId, expectedRevision, teacherId: teacher.id, reason: body.reason.trim(), idempotencyKey: key });
  ...
  } catch (error) { const code = error instanceof Error ? error.message : "STORAGE_UNAVAILABLE"; return fail(code, code === "STUDENT_NOT_FOUND" ? 404 : code === "REVISION_CONFLICT" || code === "IDEMPOTENCY_KEY_REUSED" ? 409 : 500); }
}
